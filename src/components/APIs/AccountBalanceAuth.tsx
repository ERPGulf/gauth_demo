...
const AccountBalanceAuth: React.FC = () => {
  ...
  const [description, setDescription] = useState<string>('Fetches the account balance');
  ...
  const [accountBalanceLoading, setAccountBalanceLoading] = useState<string | null>(null);
  ...
  const checkAccountBalance = async () => {
    ...
    setAccountBalanceLoading('Checking account balance...');
    try {
      const response = await axios.post(
        'https://gauth.erpgulf.com:4083/api/method/gauth_erpgulf.gauth_erpgulf.backend_server.get_account_balance',
        null,
      ...
      console.error('Error checking account balance:', error.response?.data || error.message);
    } finally {
      setAccountBalanceLoading(null);
    }
  };
      ...
      <div className="w-full md:max-w-3xl max-w-[300px] min-h-[500px] sm:min-h-[700px] bg-gray-100 p-6 sm:p-10 rounded-lg shadow-2xl">
        ...
        {masterData && (
          <>
            ...
            >
              {accountBalanceLoading ? 'Checking...' : 'Account Balance'}
            </Button>

            {/* Render account balance data only if it is available */}
            {accountBalanceData && (
              <div className="bg-gray-300 p-4 sm:p-6 mt-6 sm:mt-8 rounded-lg shadow overflow-x-auto">
